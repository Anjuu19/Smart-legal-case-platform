...
import { encryptAndSign } from './cryptoUtils';

interface StudyMaterialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (material: StudyMaterial) => void;
  materialData: StudyMaterial | null;
}
...
const StudyMaterialModal: React.FC<StudyMaterialModalProps> = ({ isOpen, onClose, onSave, materialData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        // If we are editing metadata only (no new file)
        if (materialData && !file) {
        ...
        if (file) {
            ...
            reader.onload = async (event) => {
                ...
                if (content) {
                    try {
                        const encryptedData = await encryptAndSign(content);
                        const newMaterial: StudyMaterial = {
                            id: materialData?.id || Date.now(),
                            name,
                            description,
                        ...
                        setError("File encryption failed. Please try again.");
                        setIsProcessing(false);
                    }
                }
            };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg">
        <div className="flex justify-between items-center border-b pb-3 mb-4">
          <h2 className="text-xl font-bold text-slate-800">{materialData ? 'Edit Own Study Material' : 'Add New Own Study Material'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-md">
                <div className="space-y-1 text-center">
                    ...
                    {materialData && !file && <p className="text-xs text-slate-500 font-semibold">Current file: {materialData.name}. Upload to replace.</p>}
                </div>
            </div>
          </div>
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-slate-700">Material Name / Title</label>
          ...
          <div className="flex justify-end pt-4 gap-3">
            ...
            <button type="submit" disabled={isProcessing} className="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-slate-400">
